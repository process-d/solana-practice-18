...
pub fn process_withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
    ...
    // 更新 health_factor
    let total_collateral = user.deposited_sol + user.deposited_usdc;
    let total_borrowed = user.borrowed_sol + user.borrowed_usdc;
    if total_borrowed > 0 {
        user.health_factor = (total_collateral * 10000) / total_borrowed;
    } else {

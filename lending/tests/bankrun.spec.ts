...
import { LendingProtocol } from "./lending_protocol.ts";
...
describe("Lending Smart Contract Tests", async () => {
  ...
  // 创建一个空白账本，读取lending程序，写入账本，读取pyth账户，写入账本
  ...
  // 把底层的context包装成Anchor框架熟悉的Provider接口，后续用anchor的标准方式来发送交易
  provider = new BankrunProvider(context);
  // BankrunContextWrapper: 这是 solana-bankrun 库提供的一个辅助类。
  // 作用: 它提供了一些方便的方法（比如 connection 属性），让你能更容易地访问底层的连接对象。
